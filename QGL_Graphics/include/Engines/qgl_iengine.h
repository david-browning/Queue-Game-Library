...
namespace qgl::graphics
{
   /*
     ...
     Queuing, signaling, and waiting are thread safe. Flushing and clearing are
     not. There should be only 1 thread responsible for flushing queued 
     operations. 
     Additional synchronization is required so that the owning thread waits 
     for all queue operations to complete.
    */
   ...
   class iengine
   {
      ...
      /*
       ...
       synchronization point, waits until the fence has reached the 
      ...
      /*
       ...
       Often, this means the underlying command list needs to be in the 
       recording state.
       */
