...
{
   //using cbv_descriptor_heap = descriptor_heap<D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV>;
   class LIB_EXPORT cbv_descriptor_heap : public descriptor_heap<D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV>
   {
      public:
      cbv_descriptor_heap(winrt::com_ptr<d3d_device>& dev_p,
                          size_t numEntries,
                          D3D12_DESCRIPTOR_HEAP_FLAGS heapFlag,
                          UINT nodeMask = 0);

      cbv_descriptor_heap(const cbv_descriptor_heap& r) = delete;

      cbv_descriptor_heap(cbv_descriptor_heap&& r);

      virtual ~cbv_descriptor_heap();
