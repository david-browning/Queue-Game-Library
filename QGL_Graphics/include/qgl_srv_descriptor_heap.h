...
{
   //using srv_descriptor_heap = descriptor_heap<D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV>;
   class LIB_EXPORT srv_descriptor_heap : public descriptor_heap<D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV>
   {
      public:
      srv_descriptor_heap(winrt::com_ptr<d3d_device>& dev_p,
                          size_t numEntries,
                          D3D12_DESCRIPTOR_HEAP_FLAGS heapFlag,
                          UINT nodeMask = 0);

      srv_descriptor_heap(const srv_descriptor_heap& r) = delete;

      srv_descriptor_heap(srv_descriptor_heap&& r);

      virtual ~srv_descriptor_heap();
