...
namespace qgl::graphics::gpu
{
   ...
   {
      public:
      cbv_descriptor_heap(const winrt::com_ptr<d3d_device>& dev_p,
                          size_t numEntries,
                          UINT nodeMask = 0) :
      ...
      /*
       ...
       numDescriptors: The number of descriptors for the table to include. It
       must be at least 1.

       rootIndex: index in the root signature where the table will be bound.

       baseRegister: The base shader register in the range. For example, for
       shader-resource views (SRVs), 3 maps to ": register(t3);" in HLSL.
       ...
       descriptor arrays of unknown size to not appear to overlap. For example,
       for SRVs, by extending the example in the BaseShaderRegister member
       description, 5 maps to ": register(t3,space5);" in HLSL.
