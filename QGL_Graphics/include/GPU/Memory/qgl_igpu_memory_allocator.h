...
namespace qgl::graphics::gpu
{
   /*
    ...
    deallocate(). Implementations can take different approaches. 
    Some can allocate committed resources or 1 massive heap and use a ring 
    buffer to place memory within the heap.
   ...
   class imemory_allocator
   {
      ...
      /*
       ...
       move a memory allocator, it won't actually move the memory to a new 
       location. It might just move the allocation mechanism.
       */
      imemory_allocator(imemory_allocator&& r) = delete;

      virtual ~imemory_allocator() noexcept = default;

      /*
       Aligns the buffer size, allocates memory on the GPU and returns the 
       virtual address where the memory is.
       */
      virtual D3D12_GPU_VIRTUAL_ADDRESS allocate(size_t buffSize) = 0;

      /*
       Deallocate the virtual memory that was allocated for the given GPU 
       address.
       */
