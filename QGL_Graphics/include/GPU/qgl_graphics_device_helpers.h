...
namespace qgl::graphics::helpers
{
    ...
    */
   extern bool support_tearing(dxgi_factory* fac_p);

   /*
    Returns true if the dxgi factory supports tearing on the monitor.
    */
   inline bool support_tearing(const winrt::com_ptr<dxgi_factory>& fac_p)
   {
      return support_tearing(fac_p.get());
   }
    ...
    */
   extern bool support_hdr(IDXGIOutput6* output_p);

   /*
    Returns true if the monitor supports HDR.
    HDR information is included in "desc".
    */
   inline bool support_hdr(const winrt::com_ptr<IDXGIOutput6>& output)
   {
      return support_hdr(output.get());
   }
    ...
    */
   inline UINT get_swap_flag(const device_configuration& cnfg) noexcept
   {
    ...
    */
   inline UINT get_sync_interval(const device_configuration& cnfg) noexcept
   {
   ...
   inline std::vector<winrt::com_ptr<IDXGIAdapter4>> enum_hardware_adapters(
   ...
   inline winrt::com_ptr<d3d_device> make_d3d12_device(
   ...
   inline winrt::com_ptr<d3d_cmd_queue> make_cmd_queue(
   ...
   inline d3d11_ret_t make_d3d11_dev_and_context(
   ...
   inline winrt::com_ptr<d2d_device> make_d2d_device(
   ...
   inline winrt::com_ptr<d2d_context> make_d2d_context(
   ...
   inline winrt::com_ptr<d3d_swap_chain> make_swap_chain(
      const device_configuration& config,
      DXGI_FORMAT fmt,
      UINT windowWidth,
      UINT windowHeight,
   ...
   inline void resize_swap_chain(UINT bufferCount,
      UINT windowWidth,
      UINT windowHeight,
