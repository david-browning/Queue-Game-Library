...
{
   //using sampler_descriptor_heap = descriptor_heap<D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER>;
   class LIB_EXPORT sampler_descriptor_heap : public descriptor_heap<D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER>
   {
      public:
      sampler_descriptor_heap(winrt::com_ptr<d3d_device>& dev_p,
                              size_t numEntries,
                              D3D12_DESCRIPTOR_HEAP_FLAGS heapFlag,
                              UINT nodeMask = 0);

      sampler_descriptor_heap(const sampler_descriptor_heap& r) = delete;

      sampler_descriptor_heap(sampler_descriptor_heap&& r);

      virtual ~sampler_descriptor_heap();
