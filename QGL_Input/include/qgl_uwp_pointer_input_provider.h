   ...
   class uwp_point_input_provider_traits
   {
      ...
      void hook()
      {
         m_wndSizeChangedToken = m_window_p->SizeChanged(
            { this, window_resized });
         m_pointerPressedToken = m_window_p->PointerPressed(
            { this, on_pointer_pressed });
         m_pointerReleasedToken = m_window_p->PointerReleased(
            { this, on_pointer_released });
         m_pointerMovedToken = m_window_p->PointerMoved(
      ...
      void unhook()
      {
         ...
         if (hooked)
         {
            m_window_p->PointerPressed(m_pointerPressedToken);
            m_window_p->PointerReleased(m_pointerReleasedToken);
            m_window_p->PointerMoved(m_pointerMovedToken);
